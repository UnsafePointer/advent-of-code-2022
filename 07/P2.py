...
def solve() -> int:
    ...
    total_disk_usage = calculate_size(directory=root)
    remaining_disk_space = 70_000_000 - total_disk_usage
    missing_for_update = 30_000_000 - remaining_disk_space
    current_pick = maxsize
    for directory in directories:
        directory_size = calculate_size(directory=directory)
        if directory_size >= missing_for_update:
            current_pick = min(directory_size, current_pick)
